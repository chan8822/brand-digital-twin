...
export class IdentityResolver {
  ...
  private identityLinks: Map<string, {customerId: string; confidence: number}> =
    new Map(); // hash -> link
  ...
  resolve(inputs: IdentityInput[]): ResolutionResult {
    ...
    for (const [hash, link] of this.identityLinks.entries()) {
      if (link.customerId === sourceCustomerId) {
        this.identityLinks.set(hash, {
          customerId: targetCustomerId,
          confidence: 0.9,
        }); // Reduced confidence for merged links
      }
    }

    // Link the incoming hashes to the target customer ID
    for (const h of hashes) {
      this.identityLinks.set(h, {
        customerId: targetCustomerId,
        confidence: 1.0,
      });
    }
