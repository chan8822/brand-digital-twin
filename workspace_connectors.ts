...
import { BrandSignal, TeamMember, ApprovalRequest } from "./agency_os_types";
...
export class GoogleWorkspaceConnector {
   ...
   */
  async scanCalendarSignals(tenantId: string, member: TeamMember): Promise<BrandSignal[]> {
   ...
   */
  async generateClientReportDoc(tenantId: string, clientId: string): Promise<string> {
...
export class Microsoft365Connector {
  ...
  async scanOutlookKeywords(tenantId: string): Promise<BrandSignal[]> {
    ...
    const signal: BrandSignal = {
      ...
      type: "outlook_flag",
      severity: "medium",
      message: "Urgent email subject flagged: 'Urgent: Competitor pricing change on Shopify'",
...
export class SlackConnector {
  private readonly sentSlackMessages: Array<{ channel: string; text: string; blocks?: any }> = [];
   ...
   */
  async postMessage(channel: string, text: string, blocks?: any): Promise<boolean> {
   ...
   */
  async dispatchApprovalInteractiveBlock(approval: ApprovalRequest): Promise<boolean> {
    const blocks = [
      {
        type: "section",
        text: { type: "mrkdwn", text: `*Manual Approval Needed:* ${approval.reason}` }
      },
      {
        type: "actions",
        elements: [
          { type: "button", text: { type: "plain_text", text: "Approve" }, value: "approved", action_id: "approve_btn" },
          { type: "button", text: { type: "plain_text", text: "Reject" }, value: "rejected", action_id: "reject_btn" }
        ]
      }
    ];

    return this.postMessage("#approvals", `Approval Request: ${approval.approvalId}`, blocks);
  }
}
