...
describe('ShopifyAdapter', () => {
  it('normalizes a Shopify order into canonical tables', () => {
    ...
    expect(result.order.gross_revenue).toBe(100.0);
    expect(result.order.total_discounts).toBe(10.0);
    expect(result.order.total_tax).toBe(5.0);
    expect(result.order.shipping_charged).toBe(8.0);
    ...
    expect(result.order_lines[0].order_line_id).toBe(
      'gid://shopify/LineItem/9876',
    );
    expect(result.order_lines[0].variant_id).toBe(
      'gid://shopify/ProductVariant/54321',
    );
    expect(result.order_lines[0].qty).toBe(2);
    expect(result.order_lines[0].unit_price).toBe(40.0);
    expect(result.order_lines[0].unit_cost).toBe(20.0);
  });
});
