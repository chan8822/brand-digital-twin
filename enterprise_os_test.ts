...
describe("Enterprise Agency OS (OneMCP & Bounded Contexts) Tests", () => {
  ...
  describe("Multi-Tenant Path Isolation & Security Context", () => {
    it("should resolve isolated path safely within tenant bounds and strip traversing vectors", () => {
      ...
      const traversalPath = context.resolveIsolatedPath("/data", "../../../tenant-beta/space-2/poas.sql");
  ...
  describe("OneMCP Server Specifications", () => {
    ...
    it("should validate input schema and reject call on missing required arguments", async () => {
      ...
      const response = await analystServer.callTool(context, "optimize_margins", { targetPOAS: 1.5 }, "rpc-1");
      expect(response.error).toBeDefined();
      expect(response.error?.message).toContain("Missing required parameter 'targetROI'");
    });
    ...
    it("should return -32601 on missing tools", async () => {
      ...
      const response = await analystServer.callTool(context, "non_existent_tool", {}, "rpc-1");
  ...
  describe("Unified Planner CEO Agent delegation", () => {
    it("should successfully plan and delegate strategies across OneMCP servers", async () => {
      ...
      const strategyReport = await ceoAgent.executeExecutiveStrategy("Optimize Q3 portfolio margins and correlation alerts");
      ...
      const analystReport = strategyReport['executionReports'].find((r: any) => r.agent === 'analyst');
      expect(analystReport.status).toBe("SUCCESS");
      expect(analystReport.result.targetROI).toBe(4.0);
      expect(analystReport.result.resolvedPath).toContain("/data/tenants/tenant-alpha/space-1/poas.sql");

      const riskReport = strategyReport['executionReports'].find((r: any) => r.agent === 'risk_radar');
    ...
    it("should log errors when a strategy delegator hits an unreachable agent", async () => {
      ...
      const strategyReport = await ceoAgent.executeExecutiveStrategy("Optimize strategy");

      expect(strategyReport['strategyStatus']).toBe("COMPLETED");
      
      const riskReport = strategyReport['executionReports'].find((r: any) => r.agent === 'risk_radar');
      expect(riskReport.status).toBe("FAILED");
      expect(riskReport.result).toContain("Destination agent server 'risk_radar' is unreachable");
    });
  });
});
