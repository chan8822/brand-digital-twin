...
WITH
    ...
    SELECT
      ...
      CASE
        WHEN orr.order_gross_revenue > 0
          THEN
            (
              (lm.gross_line_revenue / orr.order_gross_revenue)
              * (COALESCE(fc.shipping_cost, 0) + COALESCE(fc.marketplace_fee, 0)))
        ELSE 0
        END AS alloc_fulfillment
    FROM line_margin lm
    JOIN order_revenue orr
      USING (tenant_id, order_id)
    LEFT JOIN brand_twin.fulfillment_costs fc
      USING (tenant_id, order_id)
  ...
  line_contrib AS (
    ...
    FROM line_margin lm
    LEFT JOIN line_refunds lr
      USING (tenant_id, order_line_id)
    LEFT JOIN line_fulfillment lf
      USING (tenant_id, order_line_id)
  ),
  order_contrib AS (
  ...
  order_attribution AS (
    ...
    JOIN brand_twin.touchpoints tp
      ON
        o.customer_id = tp.customer_id
  ...
  campaign_margin AS (
    ...
    LEFT JOIN order_attribution att
      USING (tenant_id, order_id)
...
LEFT JOIN campaign_margin cm
  ON c.campaign_id = cm.campaign_id AND c.tenant_id = cm.tenant_id
