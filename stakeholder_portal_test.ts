...
describe("Multi-Stakeholder Ecosystem Portal & OS Tests", () => {
  ...
  describe("Role-Specific Dashboard Generation", () => {
    it("should generate correct widgets for Agency Owner", async () => {
      ...
      const dashboard = await dashboardEngine.getDashboardDefinition(EcosystemRole.AGENCY_OWNER, tenantId);
      expect(dashboard.title).toBe("Agency Command Center");
      
      const healthSection = dashboard.sections.find(s => s.name === "Client Portfolio Health");
      expect(healthSection).toBeDefined();
      
      const matrixCard = healthSection?.cards[0];
      expect(matrixCard?.segments?.find(s => s.status === "Thriving")?.count).toBe(1);
      expect(matrixCard?.segments?.find(s => s.status === "At Risk")?.count).toBe(1);
    });
    ...
    it("should generate correct widgets for Partner Vendor", async () => {
      ...
      const projectSection = dashboard.sections.find(s => s.name === "Active Projects");
    ...
    it("should generate correct widgets for Investor board view", async () => {
      ...
      const dashboard = await dashboardEngine.getDashboardDefinition(EcosystemRole.INVESTOR, tenantId);
      expect(dashboard.title).toBe("Board Dashboard");
      
      const summarySection = dashboard.sections.find(s => s.name === "Financial Summary");
      const revenueCard = summarySection?.cards.find(c => c.metric === "Total Revenue");
      const profitCard = summarySection?.cards.find(c => c.metric === "Net Profit");
  ...
  describe("Ecosystem Automation Workflows", () => {
    it("should automate Vendor assignments on campaign approval", async () => {
      ...
      const vendorAssignment = feed.find(f => f.actionType === "vendor_assigned");
      const kickoffScheduled = feed.find(f => f.actionType === "kickoff_scheduled");
    ...
    it("should trigger critical alerts and pause approval for critical ROI deviations", async () => {
      ...
      const pauseApproval = approvals.find(a => a.entityType === "campaign_pause" && a.entityId === brief.briefId);
    ...
    it("should auto-approve vendor invoice payments under $5000", async () => {
      ...
      await automationEngine.automatePaymentWorkflow(tenantId, asset, 3500, "stripe");
      ...
      const approvals = await db.getApprovals(tenantId);
      const invoiceApproval = approvals.find(a => a.entityType === "vendor_invoice");
      ...
      const feed = await db.getActivityFeed(tenantId);
      const paymentScheduled = feed.find(f => f.actionType === "payment_scheduled");
    ...
    it("should route vendor invoice payments over $5000 to CFO approval", async () => {
      ...
      await automationEngine.automatePaymentWorkflow(tenantId, asset, 12000, "wire");
    ...
    it("should trigger retention playbooks for high churn risk clients", async () => {
      ...
      const feed = await db.getActivityFeed(tenantId);
      const playbookAlert = feed.find(f => f.actionType === "retention_playbook_triggered");
      expect(playbookAlert).toBeDefined();
      expect(playbookAlert?.summary).toContain("Executing retention playbook");

      const approvals = await db.getApprovals(tenantId);
      const retentionSquadApproval = approvals.find(a => a.entityType === "retention_squad");
    ...
    it("should handle supplier stock protection and auto-pause approvals", async () => {
      ...
      await automationEngine.automateSupplierOptimization(tenantId, campaignId, "PROD-X", 2, true);

      const feed = await db.getActivityFeed(tenantId);
      const criticalStockAlert = feed.find(f => f.actionType === "critical_stockout_risk");
      expect(criticalStockAlert).toBeDefined();
      expect(criticalStockAlert?.summary).toContain("less than 2 days of stock");

      const approvals = await db.getApprovals(tenantId);
      const pauseRequest = approvals.find(a => a.entityType === "campaign_pause" && a.entityId === campaignId);
