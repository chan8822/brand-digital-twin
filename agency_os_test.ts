...
import { CollaborationHub, ApprovalWorkflowManager, CSuiteFinancialEngine } from "./agency_os";
import { TeamMember, ClientProfile } from "./agency_os_types";
...
describe("Agency OS Collaboration & Strategy Suite", () => {
  ...
  describe("Approvals Workflow Manager", () => {
    it("should route governance queue items to the approval registry and allow sign-off by authorized roles", async () => {
      ...
      const approval = await approvalsMgr.queueForManualReview(req, ctx, "Low confidence score", "cmo");
      ...
      const success = await approvalsMgr.processApproval(approval.approvalId, cmoUser, true, "Looks good to me");
  ...
  describe("C-Suite Portfolio Financials", () => {
    it("should calculate correct aggregate MRR and margins, and run scenario forecasts", async () => {
      ...
      const forecast = financialEngine.runScenarioModel(stats.totalMargin, 5000, 3.0, 2000);
